...
// Copyright (c) 2015-2020 The XYZK developers
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Concrete Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nConcreteBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nConcreteBadBlockBits)) {
            // accept CONCRETE block minted with incorrect proof of work threshold
            return true;
        }
