...
class WalletTest(ConcreteTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 CCT from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 CCT in block rewards plus fees, but
        ...
        # Send 10 CCT with subtract fee from amount
        txid = self.nodes[2].sendtoaddress(address, 10, "", "")
        self.nodes[2].generate(1)
        self.sync_all([self.nodes[0:3]])
        ...
        # Sendmany 10 CCT with subtract fee from amount
        txid = self.nodes[2].sendmany('from1', {address: 10}, 0, "")
        self.nodes[2].generate(1)
        self.sync_all([self.nodes[0:3]])
        node_2_bal -= Decimal('10')
